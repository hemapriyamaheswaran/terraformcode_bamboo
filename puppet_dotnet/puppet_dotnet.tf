resource "openstack_compute_instance_v2" "bamboo_puppet_dotnet_windows" {
 name            = "Bamboo_puppet_dotnet_windows"
...
resource "openstack_compute_floatingip_associate_v2" "fip_28" {
  ...
  instance_id = "${openstack_compute_instance_v2.bamboo_puppet_dotnet_windows.id}"
        }
