resource "openstack_compute_instance_v2" "Bamboo_saltstack_dotnet_windows12" {
  name            = "Bamboo_saltstack_dotnet_windows12"
...
resource "openstack_compute_floatingip_associate_v2" "fip_31" {
  ...
  instance_id = "${openstack_compute_instance_v2.Bamboo_saltstack_dotnet_windows12.id}"
        }
