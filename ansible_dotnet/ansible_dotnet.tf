resource "openstack_compute_instance_v2" "Ansible_dotnet_windows12" {
  name            = "Ansible_dotnet_windows12"
...
resource "openstack_compute_floatingip_associate_v2" "fip_29" {
  ...
  instance_id = "${openstack_compute_instance_v2.Ansible_dotnet_windows12.id}"
        }
